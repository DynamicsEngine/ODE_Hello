...
static void load_map(const char *delim, const char *prefix)
{
  for(int i = 0; i <= texture_max; ++i) texture[i] = 0;
  int llen = 80;
  char *s = (char*) alloca (strlen(prefix) + llen); // *** caution ***
  ...
  while(fgets(buf, llen, fp)){
    if(!n){
      sscanf(buf, "%d", &nfiles);
      if(nfiles > texture_max) break;
    }else{
      if(n > texture_max || n > nfiles) break;
      int len = strlen(buf);
      if(*buf && buf[len - 1] == '\x0A') buf[len - 1] = '\0';
      len = strlen(buf);
      if(*buf && buf[len - 1] == '\x0D') buf[len - 1] = '\0';
      strcpy(s, prefix);
      strcat(s, delim);
      strcat(s, buf);
  ...
  ground_texture = texture[DS_GROUND]; // must be 3
  sky_texture = texture[DS_SKY]; // must be 4
}
